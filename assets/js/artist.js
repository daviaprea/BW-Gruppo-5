...
.then(artist => {
    ...
    popularSongs.forEach((list, i) => {
        ...
        bgImage.style.background = `linear-gradient(0deg, rgba(0,0,0,.8), rgba(0,0,0,.3)), url(${artist.data[i].contributors[0].picture_xl})`;
        ...
        nameArtist.innerHTML = `${artist.data[i].contributors[0].name}`;
